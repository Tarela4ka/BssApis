...
function Library:CreateWindow(Config, Parent)
	local WindowInit = {}
	local Folder = game:GetObjects("rbxassetid://8790516160")[1]
    ...
    spawn(function ()
        while wait() do
            if Topbar.WindowName.UIGradient.Rotation < 360 then
                Topbar.WindowName.UIGradient.Rotation = Topbar.WindowName.UIGradient.Rotation +1
            elseif Topbar.WindowName.UIGradient.Rotation >= 360 then
                Topbar.WindowName.UIGradient.Rotation = 0
            end
        end
    end)
	...
	function WindowInit:CreateTab(Name)
		...
		local function GetSide(Longest)
			if Longest then
				if Tab.LeftSide.ListLayout.AbsoluteContentSize.Y >= Tab.CenterSide.ListLayout.AbsoluteContentSize.Y and Tab.LeftSide.ListLayout.AbsoluteContentSize.Y >= Tab.RightSide.ListLayout.AbsoluteContentSize.Y then
					return Tab.LeftSide
				elseif Tab.CenterSide.ListLayout.AbsoluteContentSize.Y >= Tab.RightSide.ListLayout.AbsoluteContentSize.Y and Tab.CenterSide.ListLayout.AbsoluteContentSize.Y >= Tab.LeftSide.ListLayout.AbsoluteContentSize.Y then
					return Tab.CenterSide
				elseif Tab.RightSide.ListLayout.AbsoluteContentSize.Y >= Tab.LeftSide.ListLayout.AbsoluteContentSize.Y and Tab.RightSide.ListLayout.AbsoluteContentSize.Y >= Tab.CenterSide.ListLayout.AbsoluteContentSize.Y then
		...
		Tab.LeftSide.ListLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
			...
			elseif GetSide(true).Name == Tab.CenterSide.Name then
				Tab.CanvasSize = UDim2.new(0,0,0,Tab.CenterSide.ListLayout.AbsoluteContentSize.Y + 15)
			else
				Tab.CanvasSize = UDim2.new(0,0,0,Tab.RightSide.ListLayout.AbsoluteContentSize.Y + 15)
			end
		end)

		Tab.CenterSide.ListLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
			if GetSide(true).Name == Tab.LeftSide.Name then
				Tab.CanvasSize = UDim2.new(0,0,0,Tab.LeftSide.ListLayout.AbsoluteContentSize.Y + 15)
			elseif GetSide(true).Name == Tab.CenterSide.Name then
				Tab.CanvasSize = UDim2.new(0,0,0,Tab.CenterSide.ListLayout.AbsoluteContentSize.Y + 15)
